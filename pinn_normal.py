...
class PINN:
    def __init__(self, key, layers, start_data, t_0, t_max, ic_weight=1e2, bc_weight=1e2):
        self.t_0 = t_0
        self.t_max = t_max
    ...
    def loss(self, params, batch):
        t_r, x_r, y_r, bc_batch = batch

        L_ic = self.loss_ic(params)
        L_bc = self.loss_bc(params, bc_batch)

        # Plain residual loss over all collocation points
        fu, fv, fc = vmap(self.residual, in_axes=(None, 0, 0, 0))(params, t_r, x_r, y_r)
        L_res = np.mean(fu ** 2 + fv ** 2 + 100.0 * fc ** 2)

        return self.ic_weight * L_ic + self.bc_weight * L_bc + L_res

    @partial(jit, static_argnums=(0,))
    def step(self, i, opt_state, batch):
        params = self.get_params(opt_state)
        _, g = jax.value_and_grad(self.loss)(params, batch)
        return self.opt_update(i, g, opt_state)

    def train(self, key, n_iter=20000, n_x=2048, n_bc=512):
        for it in range(n_iter):
            key, sk_r, sk_bc = random.split(key, 3)
            t_r, x_r, y_r = sample_collocation(sk_r, n_x, self.t_0, self.t_max)
            bc_batch = sample_boundary(sk_bc, n_bc, self.t_0, self.t_max)
            batch = (t_r, x_r, y_r, bc_batch)

            self.opt_state = self.step(next(self.itercount), self.opt_state, batch)

            if it % 500 == 0:
                params = self.get_params(self.opt_state)
                L_val = self.loss(params, batch)
...
if __name__ == '__main__':
    ...
    start_data = load_cfd_ic("ic_t0.npy")
    ...
    model.train(train_key, n_iter=N_ITER, n_x=N_COLLOCATION, n_bc=N_BOUNDARY)

    onp.save('pinn_log.npy', {'loss': model.loss_log})
    ...
    with open('pinn_normal.pkl', 'wb') as f:
